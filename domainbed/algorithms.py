...
class AugSAM(ERM):
    """
    SAM + Augmentation Consistency
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams)

        self.lambda_cons = hparams.get("lambda_cons", 5.0)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        x_aug = self.augment(all_x)
        logits_aug = self.network(x_aug)

        erm_loss = F.cross_entropy(logits, all_y)

        p = F.log_softmax(logits, dim=1)
        q = F.softmax(logits_aug.detach(), dim=1)
        ...
        loss = erm_loss + self.lambda_cons * cons_loss

        loss.backward()
        self.optimizer.first_step(zero_grad=True)

        # ===== SECOND PASS =====
        logits2 = self.network(all_x)

        x_aug2 = self.augment(all_x)
        logits_aug2 = self.network(x_aug2)

        erm_loss2 = F.cross_entropy(logits2, all_y)

        p2 = F.log_softmax(logits2, dim=1)
        q2 = F.softmax(logits_aug2.detach(), dim=1)
        cons_loss2 = F.kl_div(p2, q2, reduction='batchmean')

        loss2 = erm_loss2 + self.lambda_cons * cons_loss2
...
class DCSAM(ERM):
    """
    Domain-Consistent SAM (DC-SAM)

    Motivation
    ----------
    Vanilla SAM computes a single global perturbation direction using gradients
    pooled from all domains. In multi-domain training, high-loss ("hard") domains
    dominate the gradient norm, so the perturbation budget is wasted on them and
    easy domains receive almost no flatness regularisation. DC-SAM corrects this
    by computing per-domain gradient directions and averaging them before the SAM
    ascent step — ensuring every domain contributes equally to the perturbation.

    Additionally, the consistency term operates at the **feature level** (MSE on
    the featurizer output) rather than the output level, and uses
    MixStyle-inspired augmentation (channel-wise feature-statistics swap between
    two randomly paired domains) to generate meaningful cross-domain perturbations.
    A VREx-style domain-variance penalty is also added to penalise solutions that
    work well on average but fail on individual domains.

    Total loss
    ----------
    L = CE(f(x), y)  +  λ_feat * ||φ(x) - φ(aug(x))||²  +  λ_var * Var_d[CE_d]

    where φ is the featurizer and aug(x) is the feature-statistics-perturbed view.

    Parameters (hparams)
    ----------
    rho        : SAM perturbation radius (default 0.05)
    lambda_feat: weight for feature consistency loss (default 1.0)
    lambda_var : weight for domain-variance penalty  (default 1.0)
    """
