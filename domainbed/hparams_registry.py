...
def _hparams(algorithm, dataset, random_seed):
    ...
    # AugSAM uses ResNet18 by default: SAM runs 2 forward passes per step,
    # doubling memory vs ERM — ResNet50 will OOM on CPU with 8GB RAM.
    # DCSAM also uses ResNet18 for the same reason (2 forward passes + per-domain loops).
    _hparam('resnet18', algorithm in ('AugSAM', 'DCSAM'), lambda r: algorithm in ('AugSAM', 'DCSAM'))
    # Disable AugMix for AugSAM/DCSAM: they provide their own augmentation and
    # timm.create_model('resnet50.ram_in1k') requires a network download.
    _hparam('resnet50_augmix', algorithm not in ('AugSAM', 'DCSAM'), lambda r: algorithm not in ('AugSAM', 'DCSAM'))
    ...
    elif algorithm == "AugSAM":
        _hparam('lambda_cons', 1.0, lambda r: r.choice([0.1, 1.0, 5.0]))
        _hparam('rho', 0.05, lambda r: r.choice([0.01, 0.05, 0.1]))

    elif algorithm == "DCSAM":
        # SAM perturbation radius
        _hparam('rho',         0.05, lambda r: r.choice([0.01, 0.05, 0.1]))
        # Feature-level consistency loss weight
        _hparam('lambda_feat', 1.0,  lambda r: r.choice([0.1, 0.5, 1.0, 2.0]))
        # Domain-variance (VREx-style) penalty weight
        _hparam('lambda_var',  1.0,  lambda r: r.choice([0.1, 0.5, 1.0, 2.0]))
    ...
    elif algorithm in ('AugSAM', 'DCSAM'):
        # Both run 2 forward passes per step — cut batch size to avoid OOM on CPU.
